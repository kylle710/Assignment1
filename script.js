...
for (const character of youngCharacters) 
{
  console.log(character.name);
  const li = document.createElement("li");
  li.textContent = character.name;
...
Names(characters, "function-list");
...
function filteredAge(array, ageThreshold, targetId) 
{
  const list = document.getElementById(targetId);
...
filteredAge(characters, 40, "age-filter-list");
...
function NameError(array, ID, errorID) 
{
  ...
  for (const character of array) {
    if (!character.name) {
      const message = "Error: Character with id " + character.id + "is missing a name";
...
NameError(characters, "error-handling-list", "error-messages");
...
const brokenCharacters = [
  { id: 11, name: "Darth Maul", age: 47 },
  { id: 12, name: "Boba Fett", age: 164 },
  { id: 13, name: "Mace Windu", age: 30 },
];

NameError(brokenCharacters, "broken-array-list", "broken-array-errors");